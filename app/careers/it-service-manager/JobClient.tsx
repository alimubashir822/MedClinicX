...
export default function JobClient() {
  ...
  const [formData, setFormData] = useState({ name:'',email:'',portfolioUrl:'',primaryLang:'itil_service_mgmt',expYears:'5-8',cloudExp:'yes',resumeFile:null as File|null,resumeName:'' });
          const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitError, setSubmitError] = useState<string | null>(null);
  const [submitSuccess, setSubmitSuccess] = useState(false);

  const technicalSkills: TechSkill[] = [
    { name:'ITIL Service Management', level:'Required', color:'text-brand-cyan bg-brand-cyan/10 border-brand-cyan/20' },
    { name:'Incident, Problem & Change', level:'Required', color:'text-brand-indigo bg-brand-indigo/10 border-brand-indigo/20' },
    { name:'ITSM Tools (ServiceNow/Jira)', level:'Required', color:'text-brand-emerald bg-brand-emerald/10 border-brand-emerald/20' },
    { name:'SLA Tracking & Reporting', level:'Required', color:'text-purple-400 bg-purple-400/10 border-purple-400/20' },
    { name:'Cloud & Telemetry Basics', level:'Required', color:'text-amber-400 bg-amber-400/10 border-amber-400/20' },
    { name:'ITIL Expert / Leader Certs', level:'Preferred', color:'text-rose-400 bg-rose-400/10 border-rose-400/20' },
  ];

  const techAreas: TechArea[] = [
    { title:'Healthcare SaaS Platforms', desc:'Oversee operations, incident routing flows, and support queues stability for client clinics.', icon:<Layers className="w-5 h-5" /> },
    { title:'Clinical Support Systems', desc:'Coordinate support teams handling diagnostics, patient vitals, and billing ticket escalations.', icon:<Activity className="w-5 h-5" /> },
    { title:'Patient Portal Services', desc:'Manage user authentication incident pipelines and portal login downtime mitigation.', icon:<Cpu className="w-5 h-5" /> },
    { title:'EHR/EMR Connected Systems', desc:'Monitor system-to-system sync logs, connection alerts, and API bridge errors.', icon:<Database className="w-5 h-5" /> },
    { title:'Telemedicine Platforms', desc:'Audit real-time signaling errors and WebRTC connection logs to identify chronic platform bugs.', icon:<Server className="w-5 h-5" /> },
    { title:'AI Healthcare Applications', desc:'Optimize support ticket automation scripts, alerting logic, and operations telemetry rules.', icon:<Code className="w-5 h-5" /> },
  ];
  ...
  const resetForm = () => { setFormStep(1);setSubmitSuccess(false);setFormData({name:'',email:'',portfolioUrl:'',primaryLang:'itil_service_mgmt',expYears:'5-8',cloudExp:'yes',resumeFile:null,resumeName:''}); };
    ...
    <div className="max-w-6xl mx-auto px-6 py-12 md:py-16">
      ...
      <header className="mb-12 relative"><div className="absolute -inset-1 bg-gradient-to-r from-brand-cyan/20 to-brand-indigo/20 rounded-3xl blur-2xl opacity-40 -z-10 pointer-events-none" /><div className="glass-panel border border-brand-border rounded-3xl p-8 md:p-12"><div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
        <div className="md:col-span-8 space-y-6">
          <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-brand-cyan/15 to-brand-indigo/15 border border-brand-cyan/20 rounded-full px-4 py-1.5"><div className="w-1.5 h-1.5 bg-brand-cyan rounded-full animate-pulse" /><span className="text-[10px] font-bold text-brand-cyan uppercase tracking-widest">ITIL Service & Support Operations</span><Sparkles className="w-3 h-3 text-brand-cyan" /></div>
          <h1 className="font-display font-extrabold text-3xl sm:text-4xl lg:text-5xl text-white leading-tight">IT Service Manager Careers at Med Clinic X</h1>
          <p className="text-sm md:text-base text-gray-300 max-w-2xl leading-relaxed">Oversee IT service delivery, support operations, and system reliability across healthcare SaaS and digital health platforms.</p>
          <div className="flex flex-wrap gap-3">
            <span className="inline-flex items-center space-x-1.5 text-xs font-semibold px-3 py-1 bg-white/5 border border-brand-border rounded-lg text-white"><FileText className="w-3.5 h-3.5 text-brand-cyan" /><span>Service Management</span></span>
      ...
      <div id="job-content" className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start"><AnimatePresence mode="wait">
        {activeTab==='description' ? (
          <motion.div key="description" initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-15}} transition={{duration:0.3}} className="lg:col-span-8 space-y-10">
            <section className="space-y-4"><h2 className="font-display font-extrabold text-xl sm:text-2xl text-white">About Med Clinic X</h2><p className="text-sm text-gray-300 leading-relaxed">Med Clinic X is a healthcare technology company building secure, scalable, and reliable digital health solutions for healthcare organizations across the United States.</p><p className="text-sm text-gray-300 leading-relaxed">We develop healthcare SaaS platforms, AI-driven healthcare systems, patient portals, telemedicine solutions, automation workflows, and enterprise digital health systems that support modern clinical and operational workflows.</p><p className="text-sm text-gray-300 leading-relaxed">Our mission is to deliver stable, high-quality technology services that healthcare organizations can depend on every day.</p></section>
            <section className="space-y-4"><h2 className="font-display font-extrabold text-xl sm:text-2xl text-white">Job Overview</h2><p className="text-sm text-gray-300 leading-relaxed">We are seeking an <strong>IT Service Manager</strong> to oversee IT service delivery, support operations, and system reliability across healthcare SaaS and digital health platforms.</p><p className="text-sm text-gray-300 leading-relaxed">In this role, you will manage incident response, service performance, support workflows, and IT operations to ensure consistent and high-quality service delivery. You will collaborate with engineering, DevOps, support, and client teams to maintain operational excellence across Med Clinic X platforms.</p></section>
            <section className="space-y-6"><h2 className="font-display font-extrabold text-xl sm:text-2xl text-white">Responsibilities</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="glass-panel border border-brand-border p-5 rounded-2xl space-y-2"><div className="w-7 h-7 bg-brand-cyan/10 border border-brand-cyan/20 rounded-lg flex items-center justify-center text-brand-cyan mb-2"><Server className="w-4 h-4" /></div><h3 className="font-semibold text-white text-sm">IT Service Management</h3><ul className="text-xs text-gray-400 space-y-1.5 list-disc list-inside"><li>Manage end-to-end IT service delivery and operations.</li><li>Oversee incident, problem, and change management processes.</li><li>Ensure system uptime and service reliability controls.</li><li>Define and enforce IT service delivery standards.</li></ul></div>
              <div className="glass-panel border border-brand-border p-5 rounded-2xl space-y-2"><div className="w-7 h-7 bg-brand-indigo/10 border border-brand-indigo/20 rounded-lg flex items-center justify-center text-brand-indigo mb-2"><Layers className="w-4 h-4" /></div><h3 className="font-semibold text-white text-sm">Healthcare SaaS Support Operations</h3><ul className="text-xs text-gray-400 space-y-1.5 list-disc list-inside"><li>Support healthcare SaaS platforms and portal systems.</li><li>Manage service tickets and escalation support workflows.</li><li>Ensure timely resolution of clinical technical issues.</li><li>Maintain high service quality for medical providers.</li></ul></div>
              <div className="glass-panel border border-brand-border p-5 rounded-2xl space-y-2"><div className="w-7 h-7 bg-brand-emerald/10 border border-brand-emerald/20 rounded-lg flex items-center justify-center text-brand-emerald mb-2"><Activity className="w-4 h-4" /></div><h3 className="font-semibold text-white text-sm">Incident & Problem Management</h3><ul className="text-xs text-gray-400 space-y-1.5 list-disc list-inside"><li>Lead incident response, triages, and resolution paths.</li><li>Identify root causes of recurring platform lockouts.</li><li>Implement preventative measures and support optimizations.</li><li>Coordinate with engineering teams for hotfixes.</li></ul></div>
              <div className="glass-panel border border-brand-border p-5 rounded-2xl space-y-2"><div className="w-7 h-7 bg-purple-500/10 border border-purple-500/20 rounded-lg flex items-center justify-center text-purple-400 mb-2"><Shield className="w-4 h-4" /></div><h3 className="font-semibold text-white text-sm">Performance & Reporting</h3><ul className="text-xs text-gray-400 space-y-1.5 list-disc list-inside"><li>Monitor service performance levels and ticket KPIs.</li><li>Generate service reports for executive stakeholders.</li><li>Track SLAs and ensure HIPAA compliance guidelines.</li><li>Improve operations using dashboard analytics data.</li></ul></div>
            </div></section>
            <section className="space-y-4"><h2 className="font-display font-extrabold text-xl sm:text-2xl text-white">Qualifications</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-3"><h3 className="font-semibold text-white text-xs uppercase tracking-wider text-brand-cyan">Required</h3><ul className="text-xs text-gray-400 space-y-2 list-disc list-inside"><li>Bachelor’s degree in IT, Computer Science, or related field.</li><li>Experience in IT service management or IT support operations.</li><li>Strong understanding of service frameworks (ITIL preferred).</li><li>Experience managing technical support or ticketing teams.</li><li>Ability to lead teams in high-pressure outage environments.</li></ul></div><div className="space-y-3"><h3 className="font-semibold text-white text-xs uppercase tracking-wider text-brand-indigo">Preferred</h3><ul className="text-xs text-gray-400 space-y-2 list-disc list-inside"><li>Experience in US healthcare or healthcare SaaS environments.</li><li>Familiarity with HIPAA compliance and healthcare IT systems.</li><li>Experience managing enterprise-scale service environments.</li><li>Knowledge of cloud operations, DevOps, and alert tooling.</li><li>ITIL certification (Foundation or higher).</li></ul></div></div></section>
            <section className="space-y-4"><h2 className="font-display font-extrabold text-xl sm:text-2xl text-white">Why Join Med Clinic X?</h2><p className="text-sm text-gray-300 leading-relaxed">At Med Clinic X, you will ensure the reliability of healthcare technology that supports real patients and providers. You will:</p><ul className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs text-gray-400">{['Manage IT services for US healthcare organizations','Improve system reliability and support operations','Work with engineering and DevOps teams','Ensure high-quality service delivery','Contribute to healthcare operational excellence'].map((item,i)=>(<li key={i} className="flex items-start space-x-2"><CheckCircle className="w-4 h-4 text-brand-emerald shrink-0 mt-0.5" /><span>{item}</span></li>))}</ul></section>
          </motion.div>
        ) : (
