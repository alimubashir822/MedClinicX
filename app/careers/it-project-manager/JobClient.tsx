...
export default function JobClient() {
  ...
  // PM Sandbox State
  const [agileSetup, setAgileSetup] = useState({
    criticalPath: "",
    resourceConstraint: "",
    hipaaRisk: ""
  });
  ...
  const technicalSkills: TechSkill[] = [
    { name: "Agile & Scrum Sprints Management", level: "Required", color: "text-brand-cyan bg-brand-cyan/10 border-brand-cyan/20" },
    { name: "Project Tracking (Jira, Asana, MS Project)", level: "Required", color: "text-brand-indigo bg-brand-indigo/10 border-brand-indigo/20" },
    { name: "SDLC & Milestone Scoping", level: "Required", color: "text-brand-emerald bg-brand-emerald/10 border-brand-emerald/20" },
    { name: "Healthcare SaaS & IT Systems", level: "Required", color: "text-purple-400 bg-purple-400/10 border-purple-400/20" },
    { name: "PMP or Certified Scrum Master (CSM)", level: "Highly Valued", color: "text-amber-400 bg-amber-400/10 border-amber-400/20" },
    { name: "EHR/EMR Systems Integration Projects", level: "Preferred", color: "text-rose-400 bg-rose-400/10 border-rose-400/20" },
  ];

  const techAreas: TechArea[] = [
    { title: "Healthcare SaaS platforms", desc: "Oversee project scopes delivering clinical dashboards and user billing portal modules.", icon: <Globe className="w-5 h-5" /> },
    { title: "Clinical software systems", desc: "Manage backlogs mapping patient intake forms and scheduler database tables.", icon: <FileSpreadsheet className="w-5 h-5" /> },
    { title: "EHR/EMR integrations", desc: "Lead interface mapping sprints linking client systems safely via standard pipelines.", icon: <Database className="w-5 h-5" /> },
    { title: "Telemedicine platforms", desc: "Co-deliver milestones ensuring WebRTC streams and appointment calendars deploy securely.", icon: <HeartPulse className="w-5 h-5" /> },
    { title: "AI healthcare solutions", desc: "Coordinate task paths for automated intake LLM pipelines and clinical validation reviews.", icon: <Brain className="w-5 h-5" /> },
    { title: "Healthcare automation systems", desc: "Plan sprint phases integrating clinic webhooks with hospital backend schedulers.", icon: <Activity className="w-5 h-5" /> },
  ];
  ...
  const validateAgile = () => {
    ...
    // Correct PM choices:
    // criticalPath -> seven_days (Task A [3d] and B [2d] run in parallel; min time is max(3,2) + Task C [4d] = 7d)
    // resourceConstraint -> shift_load (Shift less critical assets; pair mid-level dev with senior to protect deadline)
    // hipaaRisk -> deny_prod (Deny access to production PHI; enforce fully de-identified/synthetic datasets for debugging)
    
    const correctPath = agileSetup.criticalPath === "seven_days";
    const correctConstraint = agileSetup.resourceConstraint === "shift_load";
    const correctRisk = agileSetup.hipaaRisk === "deny_prod";
    
    if (correctPath && correctConstraint && correctRisk) {
      ...
      if (!correctConstraint) incorrectCount++;
      if (!correctRisk) incorrectCount++;
      
      setSandboxError(
        `Agile audit failed. ${incorrectCount}/3 project decisions mapped incorrectly. Hint: Parallel tasks overlap in time (take the maximum duration); mitigate resource blocks without extending the client release date; and strictly block developers from using live PHI locally.`
      );
    }
  };
    ...
    <div className="relative overflow-hidden min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12 md:py-16">
        ...
        <div className="mb-8 flex items-center space-x-2 text-xs">
          ...
          <span className="text-gray-600">/</span>
          <span className="text-white">IT Project Manager</span>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
              <div className="md:col-span-8 space-y-6">
                <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-brand-cyan/15 to-brand-indigo/15 border border-brand-cyan/20 rounded-full px-4 py-1.5 shadow-lg shadow-brand-cyan/5">
                  ...
                  <span className="text-[10px] font-bold text-brand-cyan uppercase tracking-widest">Management Career</span>
                ...
                <p className="text-sm md:text-base text-gray-300 max-w-2xl leading-relaxed">
                  Lead and manage healthcare technology projects across SaaS platforms, AI systems, and digital health solutions. Coordinate cross-functional sprints for US enterprise networks.
                </p>
            ...
            {activeTab === "description" ? (
              ...
              >
                ...
                <section className="space-y-4">
                  ...
                  <p className="text-sm text-gray-300 leading-relaxed">
                    We design and develop healthcare SaaS platforms, AI-driven healthcare systems, patient portals, telemedicine solutions, automation workflows, and scalable digital health products for clinics, hospitals, and healthcare providers.
                  </p>
                  <p className="text-sm text-gray-300 leading-relaxed">
                    Our mission is to deliver high-quality healthcare technology solutions through structured, efficient, and scalable project execution.
                  </p>
                </section>
                ...
                <section className="space-y-4">
                  ...
                  <p className="text-sm text-gray-300 leading-relaxed">
                    We are seeking an <strong>IT Project Manager</strong> to lead and manage healthcare technology projects across SaaS platforms, AI systems, and digital health solutions.
                  </p>
                  <p className="text-sm text-gray-300 leading-relaxed">
                    In this role, you will be responsible for planning, executing, and delivering IT projects that support healthcare organizations in the United States. You will coordinate cross-functional teams including engineers, designers, product managers, and healthcare stakeholders.
                  </p>
                  <p className="text-sm text-gray-300 leading-relaxed">
                    You will ensure projects are delivered on time, within scope, and aligned with healthcare industry standards and business goals.
                  </p>
                </section>
                ...
                <section className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="glass-panel border border-brand-border p-5 rounded-2xl space-y-2">
                      ...
                      <h3 className="font-semibold text-white text-sm">Project Planning & Sprints</h3>
                      <ul className="text-xs text-gray-400 space-y-1.5 list-disc list-inside">
                        <li>Lead end-to-end IT project lifecycles.</li>
                        <li>Define project scope, roadmap milestones, and charts.</li>
                        <li>Coordinate daily developer standups and sprints.</li>
                        <li>Mitigate risk bottlenecks and dependency delays.</li>
                      </ul>
                    </div>
                    ...
                    <div className="glass-panel border border-brand-border p-5 rounded-2xl space-y-2">
                      ...
                      <h3 className="font-semibold text-white text-sm">Healthcare SaaS Delivery</h3>
                      <ul className="text-xs text-gray-400 space-y-1.5 list-disc list-inside">
                        <li>Coordinate development of SaaS apps and portals.</li>
                        <li>Lead AI-driven intake and clinical integrations.</li>
                        <li>Support EHR/EMR pipeline connection projects.</li>
                        <li>Verify alignment with clinical practice workflows.</li>
                      </ul>
                    </div>
                    ...
                    <div className="glass-panel border border-brand-border p-5 rounded-2xl space-y-2">
                      ...
                      <h3 className="font-semibold text-white text-sm">Team Coordination</h3>
                      <ul className="text-xs text-gray-400 space-y-1.5 list-disc list-inside">
                        <li>Align engineering, product, and UI/UX design goals.</li>
                        <li>Bridge communication with healthcare providers.</li>
                        <li>Facilitate Agile project scope reviews.</li>
                        <li>Audit backlog grooming and resource allocation.</li>
                      </ul>
                    </div>
                    ...
                    <div className="glass-panel border border-brand-border p-5 rounded-2xl space-y-2">
                      ...
                      <h3 className="font-semibold text-white text-sm">Risk, Quality & Compliance</h3>
                      <ul className="text-xs text-gray-400 space-y-1.5 list-disc list-inside">
                        <li>Identify risks and coordinate fallback options.</li>
                        <li>Ensure project compliance with HIPAA standards.</li>
                        <li>Align sprints with QA verification requirements.</li>
                        <li>Monitor project KPIs and delivery schedules.</li>
                      </ul>
                    </div>
                  </div>
                </section>
                ...
                <section className="space-y-4">
                  ...
                  <p className="text-sm text-gray-300 leading-relaxed">
                    At Med Clinic X, you will lead impactful healthcare technology projects that directly improve patient care. You will:
                  </p>
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs text-gray-400">
                    <li className="flex items-start space-x-2">
                      ...
                      <span>Manage real healthcare technology deployments in the US</span>
                    </li>
                    <li className="flex items-start space-x-2">
                      <CheckCircle className="w-4 h-4 text-brand-emerald shrink-0 mt-0.5" />
                      <span>Work directly with engineering, product, and design teams</span>
                    </li>
                    <li className="flex items-start space-x-2">
                      <CheckCircle className="w-4 h-4 text-brand-emerald shrink-0 mt-0.5" />
                      <span>Lead AI and SaaS-driven healthcare initiatives</span>
                    </li>
                    <li className="flex items-start space-x-2">
                      <CheckCircle className="w-4 h-4 text-brand-emerald shrink-0 mt-0.5" />
                      <span>Build a strong career in healthcare IT leadership</span>
                    </li>
                  </ul>
                </section>
              </motion.div>
                ...
                <div className="glass-panel border border-brand-border rounded-3xl p-6 md:p-8">
                  ...
                  <div className="flex items-center justify-between mb-8 border-b border-brand-border pb-4 font-mono text-[10px]">
                    <div className="flex space-x-4">
                      ...
                      <span className={formStep === 3 ? "text-brand-cyan font-bold" : "text-gray-500"}>03. Project Sandbox</span>
                  ...
                  {submitSuccess ? (
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <p className="text-sm text-gray-400 max-w-md mx-auto leading-relaxed">
                          Thank you for demonstrating your Agile scheduling, resource allocation, and HIPAA risk mitigation skills in our PM sandbox.
                        </p>
                      </div>
                      ...
                      <button
                        onClick={() => {
                          ...
                          setAgileSetup({ criticalPath: "", resourceConstraint: "", hipaaRisk: "" });
                          setFormData({
                            name: "",
                            email: "",
                            portfolioUrl: "",
                    ...
                    <>
                          ...
                          <div className="space-y-4">
                            ...
                            <div>
                              ...
                              <input
                                ...
                                placeholder="https://linkedin.com/in/gavinm"
                                className="w-full bg-brand-bg/50 border border-brand-border rounded-xl px-4 py-3 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-brand-cyan transition-colors"
                              />
                            </div>
                      ...
                      {/* STEP 2: PM Experience */}
                      {formStep === 2 && (
                        <div className="space-y-5">
                          <h3 className="font-display font-extrabold text-lg text-white">Project Management Profile</h3>
                          
                          <div className="space-y-4">
                            <div>
                              <label className="block text-xs font-semibold text-gray-300 mb-1.5">Primary Project Tracking Tool</label>
                              <select
                                value={formData.primaryPmTool}
                                onChange={(e) => setFormData(prev => ({ ...prev, primaryPmTool: e.target.value }))}
                            ...
                            <div>
                              <label className="block text-xs font-semibold text-gray-300 mb-1.5">Years of experience in IT Project Management</label>
                              <select
                                value={formData.pmYears}
                                onChange={(e) => setFormData(prev => ({ ...prev, pmYears: e.target.value }))}
                                className="w-full bg-brand-bg border border-brand-border rounded-xl px-4 py-3 text-sm text-white focus:outline-none focus:border-brand-cyan transition-colors"
                              >
                            ...
                            <div>
                              <label className="block text-xs font-semibold text-gray-300 mb-1.5">Are you comfortable running structured sprint planning and backlog grooming sessions?</label>
                          ...
                          <div className="pt-4 flex justify-between">
                            ...
                            >
                              <span>Proceed to Project Sandbox</span>
                              <ArrowRight className="w-3.5 h-3.5" />
                            </button>
                          </div>
                        </div>
                      ...
                      {formStep === 3 && (
                        <div className="space-y-5">
                          <div className="flex justify-between items-start">
                            <div>
                              <h3 className="font-display font-extrabold text-lg text-white">Agile Sprint Allocation & Critical Path Sandbox</h3>
                              <p className="text-xs text-gray-400 mt-1">
                                Configure critical path timelines, resolve resource constraints, and secure PHI testing protocols for EHR registration flows.
                              </p>
                            </div>
                          ...
                          <div className="space-y-4 font-sans text-xs">
                            ...
                            <div className="bg-white/2 border border-white/5 p-4 rounded-xl space-y-3">
                              <div className="font-mono text-white text-xs font-semibold flex items-center space-x-2">
                                ...
                                <span>Parameter 1: Critical Path Dependency Resolution</span>
                              </div>
                              <p className="text-gray-400 italic text-[11px] leading-relaxed">
                                &quot;Database Schema (3 days) and UI Mockups (2 days) run in parallel, and both are required before API Development (4 days) begins. What is the critical path?&quot;
                              </p>
                              <div>
                                <select
                                  value={agileSetup.criticalPath}
                                  onChange={(e) => setAgileSetup(prev => ({ ...prev, criticalPath: e.target.value }))}
                                ...
                                >
                                  <option value="">-- Choose Critical Path --</option>
                                  <option value="four_days">4 Days (Assuming all tasks execute simultaneously)</option>
                                  <option value="seven_days">7 Days (Maximum of parallel paths [3 days] + API track [4 days]) (Correct!)</option>
                                  <option value="nine_days">9 Days (Sequential serial path: 3 + 2 + 4 days)</option>
                                </select>
                              </div>
                            </div>
                            ...
                            <div className="bg-white/2 border border-white/5 p-4 rounded-xl space-y-3">
                              <div className="font-mono text-white text-xs font-semibold flex items-center space-x-2">
                                ...
                                <span>Parameter 2: Resource Constraint Mitigation</span>
                              </div>
                              <p className="text-gray-400 italic text-[11px] leading-relaxed">
                                &quot;A developer is pulled to resolve a production hotfix, delaying UI Mockup designs by 2 days. How do you protect the sprint?&quot;
                              </p>
                              <div>
                                <select
                                  value={agileSetup.resourceConstraint}
                                  onChange={(e) => setAgileSetup(prev => ({ ...prev, resourceConstraint: e.target.value }))}
                                  className="w-full bg-brand-bg border border-brand-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-brand-cyan"
                                >
                                  <option value="">-- Choose Mitigation --</option>
                                  <option value="shift_load">Postpone secondary assets; pair mid-level dev with senior to finish core input flows on time (Correct!)</option>
                                  <option value="extend_sprint">Extend sprint by 3 days and shift client release milestone timelines</option>
                                  <option value="skip_qa">Instruct the developer to bypass security scanning and unit testing to gain hours</option>
                                </select>
                              </div>
                            </div>
                            ...
                            <div className="bg-white/2 border border-white/5 p-4 rounded-xl space-y-3">
                              <div className="font-mono text-white text-xs font-semibold flex items-center space-x-2">
                                ...
                                <span>Parameter 3: HIPAA Access & Compliance Audit</span>
                              </div>
                              <p className="text-gray-400 italic text-[11px] leading-relaxed">
                                &quot;Developers request access to production database records containing patient Protected Health Information (PHI) to debug a local issue.&quot;
                              </p>
                              <div>
                                <select
                                  value={agileSetup.hipaaRisk}
                                  onChange={(e) => setAgileSetup(prev => ({ ...prev, hipaaRisk: e.target.value }))}
                                  className="w-full bg-brand-bg border border-brand-border rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus:border-brand-cyan"
                                >
                                  <option value="">-- Choose Compliance Action --</option>
                                  <option value="deny_prod">Deny live PHI access; enforce fully de-identified / synthetic datasets for local debugging (Correct!)</option>
                                  <option value="grant_nda">Grant temporary access once developers sign a non-disclosure agreement (NDA)</option>
                                  <option value="obfuscate_manual">Allow read access, but verify developers delete saved databases manually</option>
                                </select>
                              </div>
                            </div>
                          </div>
                          ...
                          {sandboxSuccess && (
                            <div className="flex items-start space-x-2.5 p-3.5 bg-brand-emerald/10 border border-brand-emerald/20 text-brand-emerald rounded-xl text-xs">
                              ...
                              <span>Sprint parameters verified! Critical path optimized, constraint mitigated, and HIPAA data rules enforced.</span>
                            </div>
                          )}
                          ...
                          <div className="pt-4 flex justify-between">
                            ...
                            {!sandboxSuccess ? (
                              <button
                                type="button"
                                onClick={validateAgile}
                                className="inline-flex items-center space-x-2 bg-brand-cyan text-brand-bg font-bold px-5 py-3 rounded-xl hover:opacity-90 transition-opacity text-xs cursor-pointer"
                              >
                                <span>Audit Agile Decisions ({sandboxAttempts} attempts)</span>
                              </button>
                            ) : (
                              <button
                                type="button"
                      ...
                      {formStep === 4 && (
                        <div className="space-y-5">
                          ...
                          <div className="space-y-4 text-xs font-mono">
                            ...
                            <div className="bg-white/2 border border-white/5 p-4 rounded-xl space-y-2">
                              <p className="text-gray-500 uppercase text-[10px] tracking-wider mb-2 font-bold font-sans">Project Management Profile</p>
                              <p className="text-gray-300">
                                <span className="text-gray-500 font-sans">PM Platform:</span>{" "}
                                <span className="capitalize">{formData.primaryPmTool}</span>
                              </p>
                              <p className="text-gray-300"><span className="text-gray-500 font-sans">PM Experience:</span> {formData.pmYears} years</p>
                              <p className="text-gray-300"><span className="text-gray-500 font-sans">Sprint Planning Comfort:</span> {formData.agileComfort}</p>
                            </div>

                            <div className="bg-white/2 border border-white/5 p-4 rounded-xl space-y-1.5">
                              <p className="text-gray-500 uppercase text-[10px] tracking-wider mb-2 font-bold font-sans">Sandbox Sprint Validation</p>
                              <p className="text-brand-emerald flex items-center space-x-1.5">
                                <CheckCircle className="w-3.5 h-3.5" />
                                <span>7-day critical path calculation, frontend resource allocation, and HIPAA data rules validated</span>
                              </p>
                            </div>
                          </div>
                            ...
                            >
                              {isSubmitting ? (
                                <>
                                  ...
                                  <span>Allocating sprint logs...</span>
                                </>
                              ) : (
                                <>
                                  <Send className="w-3.5 h-3.5" />
                                  <span>Submit IT Project Manager Application</span>
                                </>
                              )}
                            </button>
                          </div>
