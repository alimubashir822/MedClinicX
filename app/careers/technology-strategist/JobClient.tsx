...
export default function JobClient() {
  ...
  const [formData, setFormData] = useState({ name:'',email:'',portfolioUrl:'',primaryLang:'tech_strategy',expYears:'5-8',cloudExp:'yes',resumeFile:null as File|null,resumeName:'' });
          const [isSubmitting, setIsSubmitting] = useState(false);
  const [submitError, setSubmitError] = useState<string | null>(null);
  const [submitSuccess, setSubmitSuccess] = useState(false);

  const technicalSkills: TechSkill[] = [
    { name:'SaaS Strategy & Vision', level:'Required', color:'text-brand-cyan bg-brand-cyan/10 border-brand-cyan/20' },
    { name:'AI/ML Healthcare Systems', level:'Required', color:'text-brand-indigo bg-brand-indigo/10 border-brand-indigo/20' },
    { name:'Interoperability & FHIR', level:'Required', color:'text-brand-emerald bg-brand-emerald/10 border-brand-emerald/20' },
    { name:'Enterprise Architecture', level:'Required', color:'text-purple-400 bg-purple-400/10 border-purple-400/20' },
    { name:'Cloud Platforms (AWS/Azure)', level:'Required', color:'text-amber-400 bg-amber-400/10 border-amber-400/20' },
    { name:'Digital Health Tech Advising', level:'Preferred', color:'text-rose-400 bg-rose-400/10 border-rose-400/20' },
  ];

  const techAreas: TechArea[] = [
    { title:'Healthcare SaaS Strategy', desc:'Formulate long-term roadmap priorities for our multi-tenant clinic platforms.', icon:<Layers className="w-5 h-5" /> },
    { title:'AI Healthcare Innovation', desc:'Architect clinical NLP, summarizers, and automated triage models safely.', icon:<Cpu className="w-5 h-5" /> },
    { title:'EHR/EMR Ecosystems', desc:'Strategize interoperability boundaries across Epic, Cerner, and cloud portals.', icon:<Database className="w-5 h-5" /> },
    { title:'Healthcare Cloud Platforms', desc:'Guide multi-region deployment topologies, cost optimizations, and server footprint plans.', icon:<Server className="w-5 h-5" /> },
    { title:'Patient Engagement Systems', desc:'Design patient communication models, outreach automations, and scheduling tools.', icon:<Activity className="w-5 h-5" /> },
    { title:'Enterprise Architectures', desc:'Define interface specifications, message bus topologies, and data lakes guidelines.', icon:<Code className="w-5 h-5" /> },
  ];
  ...
  const resetForm = () => { setFormStep(1);setSubmitSuccess(false);setFormData({name:'',email:'',portfolioUrl:'',primaryLang:'tech_strategy',expYears:'5-8',cloudExp:'yes',resumeFile:null,resumeName:''}); };
    ...
    <div className="max-w-6xl mx-auto px-6 py-12 md:py-16">
      ...
      <header className="mb-12 relative"><div className="absolute -inset-1 bg-gradient-to-r from-brand-cyan/20 to-brand-indigo/20 rounded-3xl blur-2xl opacity-40 -z-10 pointer-events-none" /><div className="glass-panel border border-brand-border rounded-3xl p-8 md:p-12"><div className="grid grid-cols-1 md:grid-cols-12 gap-8 items-center">
        <div className="md:col-span-8 space-y-6">
          <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-brand-cyan/15 to-brand-indigo/15 border border-brand-cyan/20 rounded-full px-4 py-1.5"><div className="w-1.5 h-1.5 bg-brand-cyan rounded-full animate-pulse" /><span className="text-[10px] font-bold text-brand-cyan uppercase tracking-widest">Enterprise & Strategy Leadership</span><Sparkles className="w-3 h-3 text-brand-cyan" /></div>
          <h1 className="font-display font-extrabold text-3xl sm:text-4xl lg:text-5xl text-white leading-tight">Technology Strategist Careers at Med Clinic X</h1>
          <p className="text-sm md:text-base text-gray-300 max-w-2xl leading-relaxed">Define and guide the long-term technology vision, architecture direction, and innovation strategy across Med Clinic X platforms.</p>
          <div className="flex flex-wrap gap-3">
            <span className="inline-flex items-center space-x-1.5 text-xs font-semibold px-3 py-1 bg-white/5 border border-brand-border rounded-lg text-white"><FileText className="w-3.5 h-3.5 text-brand-cyan" /><span>Tech Strategy</span></span>
      ...
      <div id="job-content" className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start"><AnimatePresence mode="wait">
        {activeTab==='description' ? (
          <motion.div key="description" initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-15}} transition={{duration:0.3}} className="lg:col-span-8 space-y-10">
            <section className="space-y-4"><h2 className="font-display font-extrabold text-xl sm:text-2xl text-white">About Med Clinic X</h2><p className="text-sm text-gray-300 leading-relaxed">Med Clinic X is a healthcare technology company building advanced digital health solutions that help healthcare organizations across the United States modernize their operations and patient experiences.</p><p className="text-sm text-gray-300 leading-relaxed">We develop healthcare SaaS platforms, AI-driven healthcare systems, patient portals, telemedicine solutions, automation workflows, and enterprise digital health products designed to improve efficiency, outcomes, and accessibility in healthcare.</p><p className="text-sm text-gray-300 leading-relaxed">Our mission is to define the future of healthcare technology through innovation, strategy, and scalable digital solutions.</p></section>
            <section className="space-y-4"><h2 className="font-display font-extrabold text-xl sm:text-2xl text-white">Job Overview</h2><p className="text-sm text-gray-300 leading-relaxed">We are seeking a <strong>Technology Strategist</strong> to define and guide the long-term technology vision, architecture direction, and innovation strategy across Med Clinic X platforms.</p><p className="text-sm text-gray-300 leading-relaxed">In this role, you will evaluate emerging technologies, shape product direction, and align engineering and business goals to build scalable healthcare solutions for the US market.</p><p className="text-sm text-gray-300 leading-relaxed">You will work closely with leadership, product teams, and engineering architects to design the future of healthcare technology systems.</p></section>
            <section className="space-y-6"><h2 className="font-display font-extrabold text-xl sm:text-2xl text-white">Responsibilities</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="glass-panel border border-brand-border p-5 rounded-2xl space-y-2"><div className="w-7 h-7 bg-brand-cyan/10 border border-brand-cyan/20 rounded-lg flex items-center justify-center text-brand-cyan mb-2"><Activity className="w-4 h-4" /></div><h3 className="font-semibold text-white text-sm">Technology Strategy & Vision</h3><ul className="text-xs text-gray-400 space-y-1.5 list-disc list-inside"><li>Define long-term technology strategy for healthcare SaaS platforms.</li><li>Evaluate emerging technologies in AI, cloud, and digital health.</li><li>Align technical direction with business goals.</li><li>Identify opportunities for innovation and scalability.</li></ul></div>
              <div className="glass-panel border border-brand-border p-5 rounded-2xl space-y-2"><div className="w-7 h-7 bg-brand-indigo/10 border border-brand-indigo/20 rounded-lg flex items-center justify-center text-brand-indigo mb-2"><Layers className="w-4 h-4" /></div><h3 className="font-semibold text-white text-sm">Healthcare Digital Transformation</h3><ul className="text-xs text-gray-400 space-y-1.5 list-disc list-inside"><li>Guide modernization of healthcare systems and workflows.</li><li>Develop strategies for EHR/EMR integration ecosystems.</li><li>Support AI-driven healthcare innovation initiatives.</li><li>Improve interoperability and data-driven healthcare systems.</li></ul></div>
              <div className="glass-panel border border-brand-border p-5 rounded-2xl space-y-2"><div className="w-7 h-7 bg-brand-emerald/10 border border-brand-emerald/20 rounded-lg flex items-center justify-center text-brand-emerald mb-2"><Globe className="w-4 h-4" /></div><h3 className="font-semibold text-white text-sm">Architecture & Product Alignment</h3><ul className="text-xs text-gray-400 space-y-1.5 list-disc list-inside"><li>Collaborate with architects to define system direction.</li><li>Influence product roadmap with technical insights.</li><li>Evaluate platform scalability and performance strategies.</li><li>Ensure alignment between engineering and product vision.</li></ul></div>
              <div className="glass-panel border border-brand-border p-5 rounded-2xl space-y-2"><div className="w-7 h-7 bg-purple-500/10 border border-purple-500/20 rounded-lg flex items-center justify-center text-purple-400 mb-2"><Shield className="w-4 h-4" /></div><h3 className="font-semibold text-white text-sm">Research & Innovation</h3><ul className="text-xs text-gray-400 space-y-1.5 list-disc list-inside"><li>Research healthcare technology trends and competitors.</li><li>Identify opportunities in AI, automation, and cloud healthcare.</li><li>Propose new digital health solutions and frameworks.</li><li>Support proof-of-concept and innovation initiatives.</li></ul></div>
            </div></section>
            <section className="space-y-4"><h2 className="font-display font-extrabold text-xl sm:text-2xl text-white">Qualifications</h2><div className="grid grid-cols-1 md:grid-cols-2 gap-8"><div className="space-y-3"><h3 className="font-semibold text-white text-xs uppercase tracking-wider text-brand-cyan">Required</h3><ul className="text-xs text-gray-400 space-y-2 list-disc list-inside"><li>Bachelor’s or Master’s degree in Computer Science, Engineering, Business, or related field.</li><li>Experience in technology strategy, architecture, or senior consulting roles.</li><li>Strong understanding of healthcare IT or SaaS platforms.</li><li>Ability to connect business strategy with technical execution.</li><li>Strong analytical and communication skills.</li><li>Experience working with cross-functional leadership teams.</li></ul></div><div className="space-y-3"><h3 className="font-semibold text-white text-xs uppercase tracking-wider text-brand-indigo">Preferred</h3><ul className="text-xs text-gray-400 space-y-2 list-disc list-inside"><li>Experience in US healthcare or healthcare SaaS environments.</li><li>Background in digital health transformation projects.</li><li>Familiarity with HIPAA and healthcare compliance frameworks.</li><li>Experience in enterprise consulting or technology advisory roles.</li><li>Knowledge of EHR/EMR systems and interoperability standards.</li><li>Experience working with executive-level stakeholders.</li></ul></div></div></section>
            <section className="space-y-4"><h2 className="font-display font-extrabold text-xl sm:text-2xl text-white">Why Join Med Clinic X?</h2><p className="text-sm text-gray-300 leading-relaxed">At Med Clinic X, you will shape the strategic direction of healthcare technology innovation. You will:</p><ul className="grid grid-cols-1 md:grid-cols-2 gap-3 text-xs text-gray-400">{['Influence the future of US healthcare technology','Define strategy for AI and SaaS platforms','Work with leadership and architecture teams','Drive digital health transformation initiatives','Help build scalable healthcare ecosystems'].map((item,i)=>(<li key={i} className="flex items-start space-x-2"><CheckCircle className="w-4 h-4 text-brand-emerald shrink-0 mt-0.5" /><span>{item}</span></li>))}</ul></section>
          </motion.div>
        ) : (
          <motion.div key="apply" initial={{opacity:0,y:15}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-15}} transition={{duration:0.3}} className="lg:col-span-8">
            <div className="glass-panel border border-brand-border rounded-3xl p-6 md:p-8">
              <div className="flex items-center justify-between mb-8 border-b border-brand-border pb-4 font-mono text-[10px]"><div className="flex space-x-4"><span className={formStep===1?'text-brand-cyan font-bold':'text-gray-500'}>01. Profile</span><span className={formStep===2?'text-brand-cyan font-bold':'text-gray-500'}>02. Experience</span><span className={formStep===3?'text-brand-cyan font-bold':'text-gray-500'}>03. Strategic Sandbox</span></div><span className="text-gray-500">Step {formStep} of 3</span></div>
