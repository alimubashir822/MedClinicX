      ...
      <AnimatePresence>
        {visible && (
          ...
          >
            ...
            <button
              onClick={() => setOpen((v) => !v)}
              className={`flex items-center gap-2 rounded-2xl border shadow-lg shadow-black/40 transition-all duration-200 ${open
                    ...
                    {tocItems.map((item) => {
                      ...
                      return (
                        <button
                          ...
                          className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-xl text-left transition-all duration-150 ${isActive
                              ? "bg-brand-cyan/12 border border-brand-cyan/20"
                              : "hover:bg-brand-bg/80 border border-transparent"
                            }`}
                        >
                          <span className={`text-[9px] font-bold shrink-0 w-5 ${isActive ? "text-brand-cyan" : "text-gray-600"
                            }`}>{item.num}</span>
                          <span className={`w-0.5 h-3.5 rounded-full shrink-0 transition-colors ${isActive ? "bg-brand-cyan" : "bg-brand-border/60"
                            }`} />
                          <span className={`text-[11px] font-semibold truncate ${isActive ? "text-white" : "text-gray-500"
                            }`}>{item.label}</span>
                          {isActive && (
      ...
      <div className="space-y-0">
        ...
        <section id="services" className="scroll-mt-20 pb-24 md:pb-28">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-5 mb-10">
            {[
              { icon: <Users className="w-5 h-5 text-brand-cyan" />, title: "Medical Practice Website Design", desc: "Professional medical websites for doctors, physicians, and healthcare practices. Designed to communicate expertise and convert visitors into patients.", tags: ["Primary care", "Specialists", "Private practices", "Medical clinics", "Healthcare groups"] },
              { icon: <Laptop className="w-5 h-5 text-brand-cyan" />, title: "Healthcare Website Development", desc: "Secure, HIPAA-conscious infrastructure, patient intake workflows, and custom medical integrations. Fast, clean Next.js/React platforms — no templates.", tags: ["Custom websites", "Medical web apps", "Patient portals", "Healthcare dashboards", "SaaS platforms"] },
              { icon: <Layers className="w-5 h-5 text-brand-cyan" />, title: "Healthcare Website Redesign", desc: "Modernize outdated platforms while preserving SEO rankings. Full speed audit, mobile optimization, and improved navigation.", features: ["Speed audit upgrades", "Responsive mobile grids", "Preserved SEO rankings"] },
              { icon: <Lock className="w-5 h-5 text-brand-cyan" />, title: "Patient Portal Development", desc: "Secure dashboards connecting providers and patients. Medical files, prescriptions, scheduling, and installment billing.", features: ["Patient login credentials", "Diagnostic reports", "Secure message lockers"] },
              { icon: <Cpu className="w-5 h-5 text-brand-cyan" />, title: "AI Healthcare Solutions", desc: "AI-powered tools to enhance patient communication, reduce admin hours, and streamline follow-up workflows.", features: ["AI Healthcare Assistants", "Appointment automation", "SMS communication flows"] },
            ].map((card, i) => (
